...
Describe 'Git hooks install/uninstall'
  ...
  setup() {
    ...
    # Get the path to soqu-audit from the spec directory
  ...
  Describe 'cmd_install'
    It 'creates hook with markers in fresh repo'
      ...
      The contents of file ".git/hooks/pre-commit" should include "soqu-audit run || exit 1"
...
EOF
    ...
    It 'creates commit-msg hook with --commit-msg flag'
      ...
      The contents of file ".git/hooks/commit-msg" should include 'soqu-audit run "$1" || exit 1'
      The contents of file ".git/hooks/commit-msg" should include "# ======== GGA END ========"
    End

    It 'commit-msg hook passes commit message file to soqu-audit run'
      "$GGA_BIN" install --commit-msg >/dev/null 2>&1
    ...
    It 'pre-commit hook does NOT have $1 argument'
      "$GGA_BIN" install >/dev/null 2>&1
      # pre-commit hook should use simple "soqu-audit run" without $1
      The contents of file ".git/hooks/pre-commit" should include "soqu-audit run || exit 1"
...
EOF
      ...
      The contents of file ".git/hooks/pre-commit" should not include "soqu-audit run"
    End
